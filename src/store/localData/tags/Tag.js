...
export default class Tag {
  constructor(raw, tags) {
    ...
    // COMPUTED
    this.children = getChildren(raw.id, tags)
...
function getChildren(id, tags) {
  if (!tags || tags[id].parent) return null

  const children = Object.values(tags)
    .filter(tag => tag.parent === id)
    .map(tag => tag.id)
