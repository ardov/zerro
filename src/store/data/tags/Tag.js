...
function getGoal(str) {
  const regexes = {
    ...
    monthly: {
      main: s => s.match(/\*\d*\/м$/),
      amount: s => +s.match(/\*\d*/)[0].slice(1), // "*5000/м" -> 5000
    },

    // Новый компьютер *5000
    target: {
      main: s => s.match(/\*\d*$/),
      amount: s => +s.match(/\*\d*/)[0].slice(1), // "*5000" -> 5000
    },

    // Отпуск *5000-11.19
    targetByDate: {
      main: s => s.match(/\*\d*-\d*.\d*$/),
      amount: s => +s.match(/\*\d*/)[0].slice(1), // "*5000-11.19" -> 5000
      date: s =>
        new Date(
          +s.match(/\.\d*/)[0].slice(1) + 2000, // "*5000-11.19" -> 2019
          +s.match(/-\d*/)[0].slice(1) - 1 // "*5000-11.19" -> 10
        ),
    },
  }
