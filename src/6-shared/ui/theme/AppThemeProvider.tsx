...
function pinScheme(root: HTMLElement, mode: TColorScheme) {
...
const ColorSchemeMetadata: FC<{ pinned: boolean }> = ({ pinned }) => {
  ...
  /* The last colour anything reads as a value: `content` is an attribute and
     ...
     It is read from the palette and not back out of the cascade, even though
     the token is written to be readable. `getComputedStyle` forces a style
     ...
     Reading the palette here keeps colours inside the module that owns them,
     which is the point — no feature code reads one. */
  return <meta name="theme-color" content={palettes[mode].background.paper} />
}
