...
import { makeInstrument, makeStore, makeUser } from '../../testing/zenmoneyTestData'
import { getRootUser, getRootUserId, getUserCurrency, getUserInstrumentId } from './users'

describe('zenmoney users', () => {
  it('derives root user and currency from data', () => {
    const data = makeStore({
      user: { 2: makeUser({ id: 2, parent: 1, currency: 2 }), 1: makeUser({ id: 1, parent: null, currency: 1 }) },
      instrument: { 1: makeInstrument({ id: 1, shortTitle: 'EUR' }), 2: makeInstrument({ id: 2, shortTitle: 'USD' }) },
    })
