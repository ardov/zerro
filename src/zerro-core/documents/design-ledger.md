...
- Zerro Core is an internal app module, not a published package, until a real
  external or headless consumer exists.
- The root `zerro-core` entrypoint is the semantic facade: constants, shared
...
- The reference engine (`createZerroEngine`) and its outbox primitives are an
  ...
  slice imports `zerro-core/infrastructure/replica/*` directly. The `api-boundary` test pins the
...
Decided and implemented: Zerro Core is an internal app module; root is
...
Status: thin wrapper over `zerro-core/demo`.
...
```txt
zerro-core/domain/zenmoney
zerro-core/domain/zenmoney/users/types
zerro-core/domain/zerro/goals
zerro-core/domain/patch
```
...
Status: `zerro-core/presentation/tag-icons` owns package-safe emoji metadata, while
