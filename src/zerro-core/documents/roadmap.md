...
and `application/session/readGraph.ts` records the important dependency edges. Flat
...
3. ✅ **Internal-module decision.** Treat Zerro Core as an internal app module, not
   ...
   facade-only invariant); keep `infrastructure/replica/outbox.ts` internal to the Redux slice.
...
- deep app imports from `zerro-core/domain/zenmoney`, `zerro-core/domain/zerro`, and tag
  presentation shims;
...
`zerro-core/redux`. The data-account command is
...
1. ✅ Generate declarations and compile a tiny external consumer using only
   the supported root entrypoint. `pnpm zerro-core:package-check` emits into a
   temporary package, then type-checks a consumer importing `zerro-core`.
...
- Track F stays limited to the existing package-check; do not enforce subpath
  allowlists while Zerro Core is an internal module.
