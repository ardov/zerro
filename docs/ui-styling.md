...
Tailwind Preflight is disabled and legacy UI's `CssBaseline` is gone, so
...
legacy UI's numeric spacing uses 8px units; Tailwind uses 4px units. Other numeric
properties are not spacing and must be checked individually.

| Existing legacy UI value        | Equivalent utility                          |
| ------------------------- | ------------------------------------------- |
| `p: 2`                    | `p-4` (16px)                                |
...
`src/6-shared/ui/theme/palette.ts`, which is the palette legacy UI's `createTheme`
...
ever apply. `--input` is legacy UI's outlined field border — 23% of the colour that
...
is the icon colour and `--action-disabled` is the one legacy UI greys a disabled
...
rewritten as a literal in a class list. legacy UI's state fills are
...
last is the sum of the selected and hover opacities, which is how legacy UI stacks
them.
...
`@utility` (`opacity-disabled`) rather than a `--color-*` alias, because legacy UI
dims a disabled menu item instead of recolouring it.
`secondary` is the semantic selected-surface token, not legacy UI's secondary brand
color. legacy UI palette paths such as `text.disabled` are not CSS color values:
use a resolved color, a semantic utility, or keep dynamic palette lookup in
`sx`. legacy UI's `color` prop supports names such as `textDisabled`, not arbitrary
...
legacy UI `Stack spacing` is a margin rule on the children, `& > * + * { margin-top }`,
...
one: `useBreakpointDown(...)` is built from that map, and keeps legacy UI's 0.05px
...
that hook; legacy UI's `useMediaQuery` is gone from the app. The queries that are not breakpoints at all — colour
...
the pixel queries legacy UI and `useBreakpointDown` run whenever the root font size
...
margins, which scale with font size. legacy UI `Typography` sets `margin: 0`, so every
...
`type-title-lg` is the 24px / 1.334 / 400 recipe matching the themed legacy UI `h5`
contract. `type-overline` intentionally has no letter-spacing: the active IBM
Plex Sans legacy UI theme does not add the Roboto-specific overline tracking.

legacy UI's typography variants carry letter-spacing only while the theme keeps
...
the owned `Button` has none either — reading legacy UI's source rather than the
rendered result would have added it to both.

Every recipe pins its font weight. legacy UI declares one on each typography variant,
...
A `ButtonBase` (and therefore `Button`, `IconButton`, and every legacy UI control
built on it) renders a native `button`, which resets `font-family` to the user
agent default. legacy UI `Typography` set the family itself; a native element with a
...
Every owned overlay now portals to the document body. While legacy UI still owned a modal, a Base UI surface opened from inside
one had to be portalled into that modal's paper, or legacy UI's focus trap pulled
focus straight back out of it. That was the file's stated exit condition —
the last legacy UI overlay converted — and it has been met. Two Base UI surfaces
...
`SideDrawer` is legacy UI's temporary `Drawer`: a full-height sheet off a side edge
over a dimmed page, square-cornered, because legacy UI reserved rounding for
...
`className`, the way legacy UI took it through `sx` or `slotProps.paper`; the height
...
stories compare the paper against legacy UI's in both themes and at both widths —
...
`NavDrawer` is legacy UI's docked `Drawer`, which is two boxes rather than one: a
...
legacy UI's right border, its `zIndex.drawer` level (now a `--z-drawer` token beside
...
zeroed because `MenuItem` zeroes it. `listItemClass` is legacy UI's `ListItemButton`
...
one label is `body1` in a regular row and `body2` in a dense one, as legacy UI's
...
it wrap — legacy UI spelled that out per call site through `slotProps`.

A row that acts is now a real `button`, where legacy UI's `ListItemButton` was a
...
panel, asks for `text-inherit` back. `ListRows` is legacy UI's `List`, and a `div` rather than a
`ul`: legacy UI's was a `ul` whose children were `div role="button"` rows and
...
legacy UI's opt-out, and has no `dense`, because legacy UI's subheader does not take
...
and legacy UI's 4px label margin turned out to sit on `ListItemText`'s root rather
than on the `primary` span inside it. `UI/List rows` compares the container,
the subheader, the row and the label against legacy UI in both densities and both
themes; the row comparison leaves out the row's own declared font, because
legacy UI's `dense` is a context that reaches the label and leaves `body1` declared
...
`Collapse` is legacy UI's, on Base UI's `Collapsible`: height from nothing to what
...
root is `display: contents`, because legacy UI animated a single box and Base UI's
...
is open follows the content instead of clipping it — which is what legacy UI's
`height: auto` at rest bought, and there is a story that says so. And legacy UI stops
...
deliberately narrow contracts, not copies of the legacy UI prop surface.

`Button.tsx` carries `Button`, `IconButton` and `ButtonBase`, and legacy UI's names
for what they take: `variant`, `color`, `size`, `fullWidth`, `startIcon`,
`edge`. What it does _not_ carry is every combination legacy UI offers.
...
table, so a pair added there is compared against legacy UI without anyone
...
ring instead of relying on a ripple. And it does not resize a `startIcon`. legacy UI
shrinks one to 20px through `& > *:nth-of-type(1)`, but that rule sits in the
earlier cascade layer while every icon in this app carries a Tailwind size utility from
the later `utilities` layer — so legacy UI has never actually resized one of these
...
`ListRow.tsx` holds the row vocabulary legacy UI spread across `MenuItem`,
...
nothing else needs it. `ListRowText` has no margin of its own: legacy UI's
...
against the legacy UI originals.

`Menu.tsx` is legacy UI's `Menu`: a popup with menu semantics, positioned against an
...
the app uses: legacy UI's default, the menu's top-left over the anchor's, and
...
the paper legacy UI drops out of a control — the menu and the select's list are the
...
anchor's own, which is where legacy UI puts it and where Base UI does not.
...
`Dialog.tsx` is legacy UI's `Dialog`: a paper centred over a dimmed page, at most
...
the paper, which is legacy UI's `container` under another name. The paper sits at
...
every theme value gets its counterpart. Nothing grows or slides: legacy UI fades a
...
its top padding when a title sits above it, which legacy UI does with a sibling rule
and so does this. `DialogActions` uses a flex `gap` where legacy UI puts a
...
bridge, because legacy UI's focus trap had to contain the child. Two Base UI
surfaces layer themselves instead.

`SmartDialog` is that dialog on a desktop and a drawer off the bottom edge on a
phone, which is how legacy UI's `Dialog` and `SwipeableDrawer` were paired here
...
`Popover.tsx` is an anchored modal at every viewport size. It retains legacy UI's
...
`placement` and `align` are legacy UI's `anchorOrigin` in the only two settings the
...
`onOpenComplete` is legacy UI's `slots.transition.onEntered`: the filter editor's
...
anchor and it is described once for both. It carries legacy UI's own 16px minimum
width and height, and legacy UI's clipped horizontal axis: an anchored paper only
...
out of, not to whatever opened the goal: under legacy UI it hung off the goal's own
...
and the below-the-anchor centred variant against legacy UI. Month stories compare
...
legacy UI's `Drawer` is gone from the app, modal and docked alike, and with it the
last drawer surface class. So are its list primitives — `List`, `ListItem`,
...
over — and nothing else asks for it. `CheckboxField` is legacy UI's
...
an inert legacy UI input inside it. It is a span now, so there is nothing to make
inert.
...
thumb, track and track opacity, which legacy UI builds out of the scheme's extremes
rather than out of the palette. And two notations that the parity stories
caught twice over: `rounded-full` computes to 9999px where legacy UI writes `50%`,
and Tailwind's opacity modifier mixes in `oklab` where legacy UI writes `rgba`.

`Tooltip`, `Chip` and `Link` are owned now as well. `Tooltip.tsx` had wrapped
legacy UI's only to make the label 14px instead of 11px, which is now simply what
the class says; the wrapper stays because every call site already imports it,
so converting one file converted thirteen. Three things about it are legacy UI's
...
changed. legacy UI named the child, so this does too.

One thing about it is deliberately not legacy UI's. Base UI groups every tooltip
...
`Chip` is a `div` even when it has an `onClick`, because that is what legacy UI
...
follow legacy UI: legacy UI renders a delete-only chip through `ButtonBase` and it comes
...
numbers came out of that measurement rather than out of the legacy UI source: the
radius and the root's type do not change with the size (legacy UI puts the smaller
...
own button reset, which is what legacy UI's `component="button"` did.
...
mixes in `oklab` where legacy UI writes `rgba` — which is how the parity stories
found them.

legacy UI's `Collapse` is gone too — it was the only transition of legacy UI's this app
...
legacy UI's `Popover` is gone from the app. Its last four callers were the floating
...
`listItemClass`, so its keyboard tests moved off the library-owned selected class and
onto the `data-selected` attribute every owned row carries — the same
highlight, named by the app rather than by legacy UI.

`OutlinedField` is legacy UI's outlined text field: the notched border with the label
...
text included. Use `startAdornment` and `endAdornment` rather than legacy UI
...
focused or filled, which is what legacy UI calls shrinking. `Field.Root` already
...
the control's own padding, and legacy UI rests it 7px lower on a medium field than on
a small one.

`multiline` renders a textarea that grows with what is typed into it, up to
`maxRows` lines. legacy UI grows one by measuring a hidden copy of the textarea on
...
`OutlinedField.stories.tsx` pins both modes against legacy UI: where the label sits
...
value. legacy UI paints the ring from the input's own focus handler and leaves the
...
from the amount field. It is the replacement for legacy UI `MenuList`/`MenuItem` in
...
budget-source rows keep a legacy UI `Switch`, made inert — it never had an
...
Every legacy UI `Menu`, `MenuList` and `MenuItem` is converted: the two context
...
`Select.tsx` is legacy UI's outlined `Select`. Its trigger goes inside
...
`onChange` hands over the value, not an event. legacy UI's `Select` reports through a
...
not to reopen the component. The tag picker's rows used to carry an legacy UI
...
`BudgetPopover` and `AmountInput` no longer import legacy UI. The shared theme still
...
two glyph sources had left two spellings of one module, and picking between them at a call
...
legacy UI's `createSvgIcon` gave these glyphs: a `1em` box, the `fontSize` and `color`
...
those three is why they look unchanged after their former source was removed.
The transaction toolbar's edit action used Material's
...
primitive dependency. legacy UI, legacy CSS runtime, and the former parity-only packages are not
...
the weight its legacy UI variant declares.
